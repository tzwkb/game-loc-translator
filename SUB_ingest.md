...
### 任务 4：环境检查
`cli.py doctor` 检查依赖/API/目录/**向量化模型路径**。异常回传明细。

向量化模型路径检查项：
- `sentence_transformers` 是否安装
- 本地缓存中向量化模型数量（0 / 1 / 多）
- **多模型** → 回传主 Agent，由 PM 选定使用哪个（ingest-agent 不可擅自决定）
- **无模型** → 回传主 Agent，询问 PM 是否下载；PM 拒绝 → 标记 `rag_enabled=false`，后续流程跳过 RAG
- **单模型** → 自动采用，继续执行
